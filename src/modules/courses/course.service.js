...
class CourseService {
	static async createCourse(courseData) {
		...
		return CourseController.create(courseData);
	}
	...
	static async getCourse(courseId, userId = null) {
		...
		if (!course.is_public && userId) {
			...
			if (!hasAccess) {
				throw new Error(
					`Access denied, User ${userId} does not have access to course ${courseId}`,
				);
			}
		} else if (!course.is_public && !userId) {
			throw new Error(
				"Access denied: Course is not public and no user ID provided",
			);
		}
	...
	static async getModules(courseId) {
		...
		return Course.getModules(courseId);
	}
